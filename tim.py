...
c <= SimpleCNNBlock(2, 64, 3, follow_with_bn=True)
c <= TransitionBlock()
c <= SimpleCNNBlock(4, 128, 3, follow_with_bn=True)
c <= TransitionBlock()
c <= SimpleCNNBlock(4, 256, 3, follow_with_bn=True)
c <= TransitionBlock()
c <= SimpleCNNBlock(4, 386, 3, follow_with_bn=True)
c <= CustomLinearBlock(lambda x: tf.reduce_mean(x, (1, 2)))
c <= SimpleLinearBlock(200, activation_fn=None)

# Mask generator + classifier
m = UNetContainer()
m <= SimpleCNNBlock(2, 48, 3, follow_with_bn=True)
m <= SimpleCNNBlock(2, 64, 3, follow_with_bn=True)
m <= TransitionBlock()
m <= SimpleCNNBlock(5, 128, 3, follow_with_bn=True)
m <= TransitionBlock()
m <= SimpleCNNBlock(5, 256, 3, follow_with_bn=True)
m <= TransitionBlock()
m <= SimpleCNNBlock(5, 386, 3, follow_with_bn=True)
m['classification_branch'] = LinearContainer()
# continue with mask generation as a default branch
m <= UpsamplerBlock(256, True, 2)
m <= UpsamplerBlock(128, True, 2)
...
m['classification_branch'] <= SimpleLinearBlock(200, activation_fn=None)
...
masker_full_loss = 0.0005*masker_l2_loss + 5*masker_area_loss + 150*masker_smoothness_loss + preservation_loss + destroyer_loss + masker_classification_loss
masker_train_op = tf.train.MomentumOptimizer(0.01, 0.9, use_nesterov=True).minimize(masker_full_loss, var_list=masker.get_own_variables())
...
classifier_train_op = tf.train.MomentumOptimizer(0.01, 0.9, use_nesterov=True).minimize(classifier_full_loss, var_list=model.get_own_variables())
...
with tf.Session() as sess:
    ...
    nt = NiceTrainer(sess,
                     bm_train=tiny_imagenet.get_train_bm(BATCH_SIZE),
                     feed_keys=[images, labels],
                     train_op=train_op,
                     bm_val=tiny_imagenet.get_val_bm(BATCH_SIZE),
                     ...
                     computed_variables={'acc': accuracy_calc_op(),
                                         'repr': utils.rep.rep_op(64, tiny_imagenet.to_bgr_img, validation_only=True)},
                     saver=tf.train.Saver(tf.global_variables()),
                     save_every=1000,
                     save_dir='temp_ckpts/tim'
                     )
    nt.restore()
    while True:
        nt.train()
