...
for e in tf.global_variables():
    print e.name, e.get_shape().as_list()

saver2 = tf.train.Saver(tf.global_variables(), write_version=saver_pb2.SaverDef.V1)
saver2.save(sess, 'alexnet/model.ckpt', write_meta_graph=False)
exit()

img_input = np.concatenate(64*(np.expand_dims(im1, 0),), 0)
t = time.time()

for e in xrange(100):
    output = sess.run(prob, feed_dict={x: img_input})

print 64*100 / (time.time()-t)
exit()
################################################################################
